...
describe('Attendance Service E2E Tests', () => {
  ...
  describe('Complete E2E Workflow', () => {
    it('should complete full attendance workflow from employee creation to report generation', async () => {
      ...
      // Step 3: 승인 대기 목록 확인
      const pendingList = await attendanceResolver.pendingApprovals('STORE-001', undefined);
      ...
      const pendingAfterApproval = await attendanceResolver.pendingApprovals('STORE-001', undefined);
      expect(pendingAfterApproval.length).toBe(0);

      // Step 6: 일별 리포트 생성
      const dailyReport = await reportResolver.dailyAttendanceReport('2024-01-01', 'STORE-001');
    ...
    it('should handle multiple stores and employees', async () => {
      ...
      // 지점별 리포트 생성
      const report1 = await reportResolver.dailyAttendanceReport('2024-01-01', 'STORE-001');
      const report2 = await reportResolver.dailyAttendanceReport('2024-01-01', 'STORE-002');
